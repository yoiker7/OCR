...
def train(opt):
    ...
    AlignCollate_valid = AlignCollate(imgH=opt.imgH, imgW=opt.imgW, keep_ratio_with_pad=opt.PAD)    # 数据对齐方式
    valid_dataset = hierarchical_dataset(root=opt.valid_data, opt=opt)      # 生成lmdb验证集
    ...
    for p in filter(lambda p: p.requires_grad, model.parameters()):
        ...
        params_num.append(np.prod(p.size()))        # np.prod()函数用来计算所有元素的乘积，对于有多个维度的数组可以指定轴，如axis=1指定计算每一行的乘积。
...
if __name__ == '__main__':
    ...
    parser.add_argument('--continue_model', default='we_need_you/initial/TPS-ResNet-BiLSTM-Attn-case-sensitive.pth', help="path to model to continue training")
    ...
    parser.add_argument('--character', type=str, default='0123456789abcdefghijklmnopqrstuvwxyz', help='character label')  # abcdefghijklmnopqrstuvwxyz
